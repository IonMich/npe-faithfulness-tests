...
from abc_faithfulness_decay import decay_fit_summaries, make_k_grid
...
def simulator_summary_from_z(
    ...
    *,
    ...
    theta = np.exp(z_np)
    t = np.linspace(0.0, 6.0, n_observations, dtype=np.float64)
    mean = theta[:, 0:1] * np.exp(-theta[:, 1:2] * t[None, :])
    x = mean + rng.normal(0.0, theta[:, 2:3], size=mean.shape)
    summaries = decay_fit_summaries(x, t, k_grid, chunk_size=50_000)
    return torch.as_tensor(summaries, dtype=torch.float32)
...
def plot_round_distances(summary: dict[str, object], outfile: Path) -> None:
    ...
    ax.plot(
        ...
        marker="o",
        linewidth=2.0,
        color=SBI_COLORS.get(summary["density_estimator"], "#2f6fbb"),
        label=f"sbi SNPE-C {summary['density_estimator']} on summary",
    )
    ax.axhline(
        float(summary["target_wasserstein"]),
        color="#111827",
        linestyle="--",
        linewidth=1.6,
        label=f"target = {float(summary['target_wasserstein']):.3f}",
    )
    ax.set_xlabel("SNPE round")
    ax.set_ylabel("mean normalized Wasserstein to grid posterior")
    ax.set_title("Summary-based sbi SNPE-C faithfulness")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run sbi SNPE-C on refined decay summaries.")
    parser.add_argument("--rounds", type=int, default=5)
    parser.add_argument("--simulations-per-round", type=int, default=25_000)
    parser.add_argument("--density-estimator", choices=["maf", "nsf", "mdn"], default="nsf")
    parser.add_argument("--training-batch-size", type=int, default=512)
    parser.add_argument("--learning-rate", type=float, default=5e-4)
    parser.add_argument("--max-num-epochs", type=int, default=220)
    ...
    parser.add_argument("--output-dir", type=Path, default=ap.SNPE_SBI_SUMMARY_RESULTS)
    parser.add_argument("--figure-dir", type=Path, default=ap.SNPE_SBI_SUMMARY_FIGURES)
...
def main() -> None:
    ...
    x_o = torch.as_tensor(
        decay_fit_summaries(y[None, :], t, k_grid, chunk_size=1)[0],
        dtype=torch.float32,
    )
    ...
    for round_index in range(1, args.rounds + 1):
        ...
        print(f"summary sbi SNPE-C round {round_index}/{args.rounds}, proposal={type(proposal).__name__}", flush=True)
        theta = proposal.sample((args.simulations_per_round,))
        x = simulator_summary_from_z(theta, seed=args.seed + round_index * 100, k_grid=k_grid)
        ...
        print(f"  W={value:.5f}, target_ratio={round_result['target_ratio']:.2f}x, pass={round_result['target_pass']}", flush=True)
        proposal = posterior
        final_z_samples = z_samples
        final_theta_samples = theta_samples

    assert posterior is not None and final_z_samples is not None and final_theta_samples is not None

    samples_npz = args.output_dir / "snpe_sbi_summary_samples.npz"
    np.savez_compressed(
        samples_npz,
        t=t,
        y=y,
        true_theta=true_theta_np,
        observed_summary=x_o.numpy(),
    ...
    summary = {
        ...
        "rounds": args.rounds,
        "target_wasserstein": args.target_wasserstein,
        "observed_summary_log_A_log_k_log_sigma": x_o.numpy().tolist(),
    ...
    round_distances_png = args.figure_dir / "snpe_sbi_summary_round_distances.png"
    corner_png = args.figure_dir / "snpe_sbi_summary_final_corner_overlay.png"
    predictive_png = args.figure_dir / "snpe_sbi_summary_final_predictive_overlay.png"
    plot_round_distances(summary, round_distances_png)
    label = f"sbi_summary_{args.density_estimator}"
    FAMILY_COLORS[label] = SBI_COLORS.get(args.density_estimator, "#2f6fbb")
    FAMILY_LABELS[label] = f"sbi summary SNPE-C {args.density_estimator}"
    plot_focused_corner(
        {label: final_theta_samples},
        reference_samples,
        true_theta_np,
        corner_png,
        title=f"summary sbi SNPE-C final posterior ({args.density_estimator})",
    )
    plot_predictive_overlay(
        samples_by_family={label: final_theta_samples},
        t=t,
        y=y,
        true_theta=true_theta_np,
    ...
    summary_json = args.output_dir / "snpe_sbi_summary_summary.json"
    ...
    print(f"final W={value:.5f}, target_ratio={final['target_ratio']:.2f}x, pass={final['target_pass']}")
